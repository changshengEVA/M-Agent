"""Store-backed transaction registry for the shared runtime kernel.

``SQLiteRuntimeStore`` is the durable authority.  The registry keeps a
same-process live cache only because the legacy runtime still mutates
...
class TransactionRegistry:
    ...
    def record_schedule_intent(
        ...
        *,
        ...
        """Durably bind a created product schedule to its source task.

        The schedule tool runs while a delegate is pending, so the task cannot
        enter ``scheduled_wait`` yet.  This command records the intent; the
        terminal turn later registers every intent and pauses atomically.
        """

        tx_id = _required_id(transaction_id, "transaction_id")
        sid = _required_id(schedule_id, "schedule_id")
        due = _required_id(due_at, "due_at")
        owner = str(owner_id or "").strip()
        from .uow import RuntimeUnitOfWork

        command = {
            "action": "record_schedule_intent",
        ...
        def mutation(record: Optional[TransactionRecord]) -> Dict[str, Any]:
            if record is None:
                raise TransactionTransitionError(
            ...
            existing = next(
                (
                    item
                    for item in record.pending_schedule_intents
            ...
            if owner:
                intent["owner_id"] = owner
            if existing is None:
                record.pending_schedule_intents.append(intent)
            elif existing != intent:
                raise TransactionTransitionError(
                    f"schedule intent conflicts with existing binding: {sid}"
                )
            return dict(command)

        current = self.get(tx_id)
        if current is None:
            raise TransactionTransitionError(f"unknown transaction: {tx_id}")
        RuntimeUnitOfWork(self).apply_transition(
            f"schedule-intent:{tx_id}:{sid}",
            command,
            tx_id,
            current.revision,
            mutation,
        )
        refreshed = self.get(tx_id)
