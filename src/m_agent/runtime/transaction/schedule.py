...
class TransactionScheduleCoordinator:
    ...
    def register(
        ...
        *,
        ...
        owner_id = str(schedule_owner_id or "").strip() or None
        transition = (
        ...
        def operation(
            ...
            if existing is not None:
                ...
                has_pending_intent = any(
                    ...
                    == sid
                    and str(item.get("due_at", "") or "").strip()
                    == due
            ...
            if record.state == TransactionState.CONTINUE:
                ...
                raise ScheduleTransitionError(
                    "schedule registration requires a running transaction "
                    "or pause(scheduled_wait)"
                )
            record.correlation.schedule_id = sid
            record.correlation.schedule_run_id = run_id
            if owner_id:
                record.correlation.schedule_owner_id = owner_id
            record.pending_schedule_intents = [
                item
                for item in record.pending_schedule_intents
            ...
            if existing is not None:
                run = OneShotScheduleRun.from_dict(existing)
