"""Direct single-tool invocation for Runtime (param LLM + direct invoke)."""
...
from m_agent.runtime.turn_support.execution_feedback import feedback_summary_from_tool_history
...
# Runtime: tools that skip the param LLM; think-layer ``instruction`` maps to invoke kwargs.
...
RUNTIME_SKIP_PARAM_INSTRUCTION_ARG: Dict[str, Optional[str]] = {
...
SKIP_PARAM_LLM_TOOLS = frozenset({REPLY_TOOL_NAME, *RUNTIME_SKIP_PARAM_INSTRUCTION_ARG.keys()})
...
def build_tool_input(
    ...
    *,
    ...
    skip_arg = RUNTIME_SKIP_PARAM_INSTRUCTION_ARG.get(name)
    if skip_arg is not None or name in RUNTIME_SKIP_PARAM_INSTRUCTION_ARG:
        if skip_arg is None:
            return {}
