...
// Legacy INV-01..INV-14 dashboard. The default UI is the P1 contract view.
...
function bindEvents() {
  ...
  elements.contractRuntimeSelect.addEventListener("change", renderContractTree);
  elements.contractRunButton.addEventListener("click", startContractRun);
  elements.contractCancelButton.addEventListener("click", cancelContractRun);
}
...
async function loadContractCatalog() {
  try {
    const payload = await requestJson("/contract/catalog");
    const coverage = isObject(payload.coverage) ? payload.coverage : {};
    const execution = isObject(payload.execution) ? payload.execution : {};
    const byRuntime = isObject(execution.executable_variants_by_runtime)
      ? execution.executable_variants_by_runtime
      : {};
    elements.p1ContractCoverage.textContent =
      `${finiteNumber(coverage.specified_core, 0)} / ` +
      `${finiteNumber(coverage.required_core, 27)} Core · ` +
      `${finiteNumber(coverage.specified_robustness, 0)} / ` +
    ...
    state.contractCatalog = payload;
    elements.contractRunButton.disabled = false;
    renderContractTree();
  } catch (error) {
    state.contractCatalog = null;
    elements.contractRunButton.disabled = true;
    elements.p1ContractCoverage.textContent =
      `P1 目录读取失败：${messageOf(error)}`;
    elements.p1ContractExecution.textContent = "可执行覆盖：未知";
    elements.p1ContractState.textContent = "不可用";
    elements.p1ContractState.className = "quiet-badge quiet-badge-danger";
    elements.contractResultSummary.textContent =
      `P1 目录不可用：${messageOf(error)}`;
    elements.contractTree.replaceChildren();
  }
}

function contractVariantResultMap() {
  const result = isObject(state.contractRun)
    ? state.contractRun.result
    : null;
  const map = new Map();
  asArray(result && result.scenarios).forEach((scenario) => {
    asArray(scenario.variants).forEach((variant) => {
      map.set(cleanText(variant.variant_id || variant.id), variant);
    });
  });
  return map;
}
...
function renderContractTree() {
  const payload = state.contractCatalog;
  if (!isObject(payload)) return;
  const runtimeId = elements.contractRuntimeSelect.value;
  const resultByVariant = contractVariantResultMap();
  elements.contractTree.replaceChildren();

  ["TX", "SP", "AT"].forEach((domain) => {
    const scenarios = asArray(payload.scenarios).filter(
      (item) => cleanText(item.domain) === domain,
    );
    const domainSection = createElement("section", "contract-domain");
    const heading = createElement("div", "contract-domain-heading");
    heading.append(
      createElement("strong", "", domain),
      createElement(
        "span",
        "count-pill",
        `${scenarios.length} 个顶层场景`,
      ),
    );
    domainSection.append(heading);

    const layers = domain === "AT"
      ? ["core", "robustness", "matcher_evaluation"]
      : ["core", "robustness"];
    layers.forEach((layer) => {
      const variants = scenarios.flatMap((scenario) =>
        asArray(scenario.variants)
          .filter((variant) => cleanText(variant.layer) === layer)
          .map((variant) => ({ scenario, variant })),
      );
      if (!variants.length) return;
      const layerGroup = createElement("div", "contract-layer");
      const layerTitle = createElement("div", "contract-layer-title");
      layerTitle.append(
        createElement(
          "span",
          "layer-badge",
          layer === "matcher_evaluation"
            ? "Matcher Evaluation"
            : layer === "robustness"
              ? "Robustness"
              : "Core Gate",
        ),
        createElement("span", "field-help", `${variants.length} variants`),
      );
      layerGroup.append(layerTitle);

      variants.forEach(({ scenario, variant }) => {
        const binding = asArray(variant.bindings).find(
          (item) => cleanText(item.runtime_id) === runtimeId,
        ) || {};
        const executed = resultByVariant.get(
          cleanText(variant.variant_id || variant.id),
        );
        const status = cleanText(
          executed ? executed.status : binding.availability,
        ) || "not_covered";
        const details = createElement("details", "contract-variant");
        if (["failed", "known_gap"].includes(status)) details.open = true;
        const summary = createElement("summary", "contract-variant-summary");
        const identity = createElement("span", "contract-variant-identity");
        identity.append(
          createElement(
            "code",
            "",
          ...
          createElement("span", "", cleanText(scenario.title)),
        );
        summary.append(identity, createStatusChip(status));
        details.append(summary);

        const body = createElement("div", "contract-variant-body");
        body.append(
          createElement(
            "p",
            "field-help",
            `${runtimeId} · ${cleanText(binding.availability) || "unknown"}`,
          ),
        );
        if (binding.known_gap) {
          body.append(
            createElement(
              "p",
              "contract-gap-copy",
              `Known Gap：${cleanText(binding.known_gap)}`,
            ),
          );
        }
        const cases = asArray(executed && executed.cases);
        cases.forEach((caseItem) => {
          const caseHeading = createElement(
            "div",
            "contract-case-heading",
          );
          caseHeading.append(
            createElement(
              "code",
              "",
              cleanText(caseItem.nodeid) || "scenario evidence",
            ),
          ...
          if (checks.length) {
            const checkList = createElement("ul", "contract-check-list");
            checks.forEach((check) => {
              const passed = check.passed === true;
              const item = createElement(
                "li",
                passed ? "contract-check-pass" : "contract-check-gap",
              );
              item.append(
                createElement("strong", "", passed ? "✓" : "!"),
                createElement(
                  "span",
                  "",
                  `${cleanText(check.id)} · ${cleanText(check.description)}`,
                ),
              );
          ...
          if (isObject(data.facts) && Object.keys(data.facts).length) {
            const facts = createElement("details", "contract-facts-detail");
            facts.append(createElement("summary", "", "查看 normalized facts"));
            facts.append(
              createElement(
                "pre",
                "",
          ...
          const trace = asArray(caseItem.trace).filter(isObject);
          if (trace.length) {
            const traceDetail = createElement(
              "details",
              "contract-trace-detail",
            );
            traceDetail.append(
              createElement(
                "summary",
                "",
                `查看 semantic trace（${trace.length} events）`,
              ),
              createTraceTimeline(trace),
            );
            body.append(traceDetail);
          }
        });
        details.append(body);
        layerGroup.append(details);
      });
      domainSection.append(layerGroup);
    });
    elements.contractTree.append(domainSection);
  });

  const total = finiteNumber(
    payload.execution &&
      payload.execution.executable_variants_by_runtime &&
      payload.execution.executable_variants_by_runtime[runtimeId],
    0,
  );
  if (!state.contractRun) {
    elements.contractResultSummary.textContent =
      `${runtimeId}：${total} 个可执行 variant；展开场景可查看预登记差距。`;
  }
}

async function startContractRun() {
  if (state.contractStarting || !state.contractCatalog) return;
  state.contractStarting = true;
  elements.contractRunButton.disabled = true;
  elements.contractCancelButton.hidden = false;
  setStatusChip(elements.contractRunState, "starting");
  elements.contractResultSummary.textContent = "正在创建 P1 运行…";
  try {
    const runtimeId = elements.contractRuntimeSelect.value;
    ...
    state.contractRun = payload;
    setStatusChip(elements.contractRunState, payload.status);
    const generation = ++state.contractPollGeneration;
    pollContractRun(payload.run_id, generation);
  } catch (error) {
    setStatusChip(elements.contractRunState, "error");
    elements.contractResultSummary.textContent =
      `P1 运行启动失败：${messageOf(error)}`;
    elements.contractCancelButton.hidden = true;
    showToast(messageOf(error), "error");
  } finally {
    state.contractStarting = false;
    elements.contractRunButton.disabled = false;
  }
}

async function pollContractRun(runId, generation) {
  if (generation !== state.contractPollGeneration) return;
  try {
    const payload = await requestJson(`/contract/runs/${encodeURIComponent(runId)}`);
    if (generation !== state.contractPollGeneration) return;
    state.contractRun = payload;
    setStatusChip(elements.contractRunState, payload.status);
    const nested = isObject(payload.result) ? payload.result : {};
    const variants = asArray(nested.scenarios).flatMap(
      (scenario) => asArray(scenario.variants),
    );
    const gaps = variants.filter(
      (variant) => cleanText(variant.status) === "known_gap",
    ).length;
    const passed = variants.filter(
      (variant) => cleanText(variant.status) === "passed",
    ).length;
    elements.contractResultSummary.textContent =
      `run=${runId} · ${passed} passed · ${gaps} known gaps · ` +
    ...
    window.setTimeout(
      () => pollContractRun(runId, generation),
      POLL_INTERVAL_MS,
    );
  } catch (error) {
    if (generation !== state.contractPollGeneration) return;
    setStatusChip(elements.contractRunState, "error");
    elements.contractResultSummary.textContent =
      `P1 状态同步失败：${messageOf(error)}`;
    elements.contractCancelButton.hidden = true;
  }
}

async function cancelContractRun() {
  const runId = cleanText(state.contractRun && state.contractRun.run_id);
  if (!runId) return;
  try {
    await requestJson(
    ...
    setStatusChip(elements.contractRunState, "cancelling");
  } catch (error) {
    showToast(messageOf(error), "error");
  }
}
