"""Executable SP contract scenarios for the current Think-life Runtime."""
...
def _run_sp_01_core(
    ...
    transaction = harness.create_legacy_transaction(
    ...
    evidence.check(
        ...
        "A valid legacy Feedback is resolved to its source transaction.",
        actual={
    ...
    evidence.check(
        ...
        evidence="ThinkLifeV1Harness.submit_stimulus",
        known_gap_key=gap,
    )
    ...
    if invalid_popped is not None:
        try:
            ...
            late_rejected = True
    evidence.check(
        "sp01.legacy_late_feedback_check",
    ...
    race_transaction = harness.create_legacy_transaction(
...
def _run_sp_04_core(
    ...
    feedback_transaction = harness.create_legacy_transaction(
        conversation_id=conversation_id,
        status=TransactionStatus.RUNNING,
    )
    harness.registry.begin_delegate(
    ...
    evidence.check(
        ...
        expected=True,
        evidence="ThinkLifeV1Harness.control_effect_sink",
        known_gap_key=gap,
    )
    ...
    evidence.check(
        ...
        expected=True,
        evidence="ThinkLifeV1Harness.set_worker_latch",
        known_gap_key=gap,
    )
    return evidence.build()
...
def _run_sp_05_core(
    ...
    evidence.check(
        "sp05_legacy_source_fields_not_overwritten",
        "Concurrent producers do not overwrite legacy transaction/delegate fields.",
        actual={
...
def _run_sp_07_core(
    ...
    original_transaction = harness.create_legacy_transaction(
    ...
    evidence.check(
        ...
        expected=True,
        evidence="ThinkLifeV1Harness.control_transaction",
        known_gap_key=gap,
    )
...
def _run_sp_08_core(
    ...
    evidence.check(
        ...
        expected=True,
        evidence="ThinkLifeV1Harness.create_conversation",
    )
    ...
    transaction_a = harness.create_legacy_transaction(
        conversation_id=conversation_a,
        status=TransactionStatus.RUNNING,
    )
    transaction_b = harness.create_legacy_transaction(
        conversation_id=conversation_b,
        status=TransactionStatus.RUNNING,
    )
    evidence.check(
    ...
    evidence.check(
        ...
        "Feedback cannot cross a conversation boundary even with valid legacy IDs.",
...
def _run_sp_01_durable_ingress_restart(
    ...
    evidence.check(
        ...
        "A destructive legacy pop still reports the original stimulus ID.",
    ...
    evidence.check(
        ...
        expected=True,
        evidence="ThinkLifeV1Harness.restart_runtime",
        known_gap_key=gap,
    )
...
_HANDLERS: Dict[
    tuple[str, str],
    Callable[
        [ThinkLifeV1Harness, EvidenceBuilder, str],
        ScenarioExecution,
    ],
] = {
...
def run_sp_scenario(
    ...
    """Drive one SP variant through production Think-life components."""
    ...
    if key not in _SP_VARIANTS:
        raise RuntimeAdapterError(
            f"think_life_v1 does not implement {scenario}/{variant}"
        )
    evidence = EvidenceBuilder(
