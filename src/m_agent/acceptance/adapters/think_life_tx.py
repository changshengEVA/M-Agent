"""Executable TX contract scenarios for the current Think-life Runtime.
...
Arrange code may seed legacy records.  Act code goes through
``ThinkLifeV1Harness`` operations or public production commands exposed by the
Harness.
"""
...
def _seed_transaction(
    ...
    *,
    ...
    """Arrange one valid legacy transaction through public Registry commands."""

    record = harness.create_legacy_transaction(
...
def _result_check(
    ...
    *,
    ...
    return evidence.check(
        ...
        actual=result.to_dict(),
        expected=expected,
        evidence=f"ThinkLifeV1Harness.{result.operation}",
...
def _run_tx_01_core(
    ...
    evidence.check(
        ...
        expected="continue",
        evidence="ThinkLifeV1Harness.load_transaction",
        known_gap_key=gap_key,
    )
    evidence.check(
...
def _run_tx_03_core(
    ...
    evidence.check(
        ...
        evidence="ThinkLifeV1Harness.list_match_candidates",
        known_gap_key=gap_key,
...
def run_tx_scenario(
    ...
    if handler is None:
        raise RuntimeAdapterError(
            "think_life_v1 TX adapter does not implement "
            f"{scenario}/{variant}"
        )
