...
def main():
    ...
    try:
        # 选择一个实体进行测试
        test_entity = "changshengEVA"
        
        print(f"   测试解析实体: {test_entity}")
        result = service.resolve_and_apply(test_entity)
        
        print(f"   解析结果:")
        print(f"     判定类型: {result.decision.resolution_type.value}")
        print(f"     源实体: {result.decision.source_entity_id}")
        
        if result.decision.target_entity_id:
            print(f"     目标实体: {result.decision.target_entity_id}")
        
        print(f"     置信度: {result.decision.confidence:.2f}")
        print(f"     策略: {result.decision.strategy_name}")
        print(f"     应用成功: {result.applied}")
        print(f"     Library更新: {result.library_updated}")
        print(f"     KG操作执行: {result.kg_operation_performed}")
        
        if result.error:
            print(f"     错误: {result.error}")
        
    except Exception as e:
        print(f"   ✗ 测试单个实体解析失败: {e}")
        import traceback
        traceback.print_exc()
        return False
    ...
    try:
        # 选择几个实体进行批量测试
        test_entities = ["Peking_University", "PKU", "ZQR", "galgame"]
        
        print(f"   批量解析实体: {test_entities}")
        results = service.batch_resolve_and_apply(test_entities)
        
        print(f"   批量解析结果:")
        for i, result in enumerate(results):
            entity_id = test_entities[i]
            print(f"     {entity_id}: {result.decision.resolution_type.value} "
                  f"(成功: {result.error is None})")
    ...
    try:
        # 保存Library数据到文件
        print(f"   保存Library数据到: {entity_library_path}")
        save_success = service.entity_library.save_to_path(str(entity_library_path))
        
        if save_success:
            print(f"   ✓ Library数据保存成功")
            
            # 检查保存的文件
            library_files = list(entity_library_path.glob("*.json"))
            print(f"   生成的实体库文件数量: {len(library_files)}")
            
            if library_files:
                print(f"   实体库文件:")
                for file in library_files[:5]:  # 只显示前5个
                    file_size = file.stat().st_size
                    print(f"     - {file.name} ({file_size} bytes)")
                
                if len(library_files) > 5:
                    print(f"     ... 还有 {len(library_files) - 5} 个文件")
                
                # 检查一个文件的内容
                if library_files:
                    sample_file = library_files[0]
                    try:
                        with open(sample_file, 'r', encoding='utf-8') as f:
                            sample_data = json.load(f)
                        print(f"\n   示例文件内容 ({sample_file.name}):")
                        print(f"     ID: {sample_data.get('ID')}")
                        print(f"     name: {sample_data.get('name')}")
                        print(f"     别名数量: {len(sample_data.get('alias_names', []))}")
                        print(f"     embedding长度: {len(sample_data.get('embedding', [])) if sample_data.get('embedding') else 0}")
                    except Exception as e:
                        print(f"     读取示例文件失败: {e}")
            else:
                print(f"   ✗ 保存后实体库目录仍然为空")
        else:
            print(f"   ✗ Library数据保存失败")
        
    except Exception as e:
        print(f"   ✗ 保存Library数据失败: {e}")
        import traceback
        traceback.print_exc()
