...
> 基线版本：v0.2.0  
> 确认日期：2026-08-06  
> 修订说明：自 v0.4 线起重排为 v0.4.1—v0.4.3；v0.2.0—v0.3.x 历史交付保持不变  
...
| **v0.4.1** | 工具情景记忆维护系统 | 建成可由模型主动调用的情景记忆维护与深召回能力 | 工具情景记忆维护闭环（录入/维护/深召回） |
| **v0.4.2** | 系统情景记忆维护系统 | 建成由 Runtime 驱动的情景记忆维护与浅召回能力 | 系统情景记忆维护闭环（录入/维护/浅召回） |
| **v0.4.3** | 图结构实体档案记忆维护系统 | 建成面向实体档案的图结构记忆维护能力 | 实体档案记忆维护闭环（图结构索引与维护） |
| **v0.5.0** | 经验子系统 | 建成自洽的经验能力闭环 | Flush 录入、仅系统召回；与情景记忆解耦 |
| **v0.6.0** | 长时记忆接入与主系统稳定 | 已自立的记忆子系统拼进主链，主系统可长期稳定运行 | Flush 只触发不内嵌算法、决策前评估注入、隔离/导出/删除、故障降级 |
...
### v0.4 线：长时记忆维护系统（分版推进）

v0.4 不再作为单一大版本一次性交付，而是拆成三个可独立放行的小版本。各版只先锁定主题与目标；具体实现、接口与放行细则待进入该版本时再定。

共通边界（全线适用）：

- Working Memory（短时对话连续性）已在既有运行时具备，本线不重做 WM；
- 事务归因继续以 **conv 内归因** 为准，不扩展开放事件流 Attention；
- Flush / Runtime 只负责边界与触发，不内嵌各记忆子系统的维护与检索算法；
- 各子系统应可独立演进、单独验收，并避免并行两套语义。

#### v0.4.1：工具情景记忆维护系统

**主题**

实现可由模型主动调用的工具情景记忆维护系统。

**目标**

建成工具侧情景记忆的维护与深召回能力闭环，作为长时情景记忆的第一块可独立交付能力。

当前接口草案见 [v0.4.1 记忆平台接口设计](architecture/memory-platform-v0.4.1.zh-CN.md)。

#### v0.4.2：系统情景记忆维护系统

**主题**

实现由 Runtime 驱动的系统情景记忆维护系统。

**目标**

建成系统侧情景记忆的维护与浅召回能力闭环，与工具深召回职责分离。

#### v0.4.3：图结构实体档案记忆维护系统
...
| 工具情景记忆维护 | 模型主动调用的情景记忆维护/深召回 | v0.4.1 |
| 系统情景记忆维护 | Runtime 驱动的情景记忆维护/浅召回 | v0.4.2 实现，v0.6.0 主链拼接 |
| 图结构实体档案记忆维护 | 实体档案的图结构记忆维护 | v0.4.3 |
...
- **工具情景记忆（v0.4.1）：**模型想起后主动查询/维护。
- **系统情景记忆（v0.4.2）/ 经验召回（系统）：**Runtime 在决策前评估是否注入。
- **Flush：**只负责边界与触发，不拥有记忆算法细节。
- 各记忆子系统保持自洽，避免把复杂记忆构建链路全塞进 flush。
...
- **后续 v0.3.x：**消除剩余内部旁路后，才可离开 v0.3 线进入 v0.4（自 v0.4.1 起分版推进记忆维护系统）；
