...
> 状态快照：2026-08-02（**P8 完成**；**P8 后 R1～R3 完成**；**R4 生产机制已接线、观察期进行中**：Chat API 使用 Composite 双引擎 host，新 transaction 默认 `langgraph_v1`，既有 transaction 按持久化 `runtime_engine` 继续执行；R5 尚未开始）<br>
...
> **当前结论：P0～P8 已完成；生产接线 R1～R3 已完成，R4 机制已落地但仍在
> 观察期。双 Runtime 共享矩阵 38/38 variant 可执行且 56/56 验收用例全绿。
> Chat API 已持 `CompositeRuntimeHost`，新 transaction 默认 `langgraph_v1`；
> `M_AGENT_DEFAULT_RUNTIME_ENGINE=think_life_v1` 可回退新 transaction，但不会迁移
> 已有 transaction。R5 必须等待观察期结束且只读 audit 报告
> `legacy_retirement_ready=true`，当前不得删除 ThinkLife transaction 循环。
...
- 共享 Runtime Harness、ThinkLife adapter、CLI、Web 分层工作区、结构化报告和
  [P1 分层差距表](think-life-p1-gap-matrix.zh-CN.md)均已完成。
...
- LangGraph Runtime 已在 P8 完成完整矩阵：持久 SQLite checkpointer、SP/AT/Scene
  ...
  收缩进入稳定观察期。
- 产品 RuntimeHost 已升级为 Composite 双引擎：显式 transaction 与已有活跃
  conversation 按 Store 中的 `runtime_engine` 回到原 engine；默认 engine 只在创建新
  transaction 时使用。checked-in 产品默认值为 `langgraph_v1`，env 覆盖优先于 YAML。
- LangGraph 持久 checkpoint 已改为 fail-fast 契约：缺少 SQLite saver、缺少持久化路径
  或 schema marker 不兼容时拒绝启动；内存 saver 只能显式用于测试，health 提供
  backend/durable/schema 观察面。
- 一次性 Schedule 保存原 transaction origin，并使用稳定 run/delivery ID 恢复原
  transaction；产品 Flush 使用逐 engine 不可变 snapshot（独立 `through_seq` 与 eligible
  revision）、单库 `FlushCoordinator` 和持久 Composite saga，Dialogue/RAG 物化 payload
  在提交前进入 outbox，故障或重启后继续同一 `flush_id`。
- R4/R5 已有两个运维入口：`scripts/run_runtime_migration_gate.py` 执行可重复的双
  Runtime 门禁，`scripts/audit_runtime_cutover.py` 只读清点 legacy recoverable work。
...
| ThinkLife Runtime | **P7 已完成；R5 兼容期** | 四态与 tombstone；activation/delegate；revision/CAS；幂等 ledger；SQLite Transaction/Scene/Flush；Pause/Delete/Restore；Store-backed Stimulus Pool；Schedule delivery 幂等；effect ledger + Feedback outbox；capability delivery guarantee；35/35 全绿 | 继续完成其存量工作；观察期及 `legacy_retirement_ready` 门满足后才可收缩 |
| 跨 Runtime Harness / Host | **P8/R3 已完成** | 共用参考 Store；双 Runtime 完整矩阵；process 重建与 checkpoint 持久化；生产 Composite host 按 transaction 归属分发 | R4 持续观察和回退演练 |
| LangGraph Runtime | **P8 完成；R4 默认已接线** | 持久 SQLite checkpoint fail-fast/health；SP/AT/Scene 共享外层；稳定 Schedule；原子 Flush；38/38 variant 可执行；新 transaction 产品默认 | 完成 R4 观察期；R5 不属于当前完成项 |
...
- [ThinkLife P1 分层差距表](think-life-p1-gap-matrix.zh-CN.md)。
...
[P1 分层差距表](think-life-p1-gap-matrix.zh-CN.md)为准。
...
- [contracts.py](../../../src/m_agent/runtime/domain/contracts.py)
- [transaction_registry.py](../../../src/m_agent/runtime/transaction/registry.py)
- [runtime.py](../../../src/m_agent/runtime/langgraph/runtime.py)
...
- [inbox.py](../../../src/m_agent/runtime/perception/inbox.py)
- [gateway.py](../../../src/m_agent/runtime/perception/gateway.py)
- [drainer.py](../../../src/m_agent/runtime/dispatch/drainer.py)
...
- [attributor.py](../../../src/m_agent/runtime/perception/attributor.py)
- [core.py](../../../src/m_agent/layers/thinking/core.py)
- [loop.py](../../../src/m_agent/runtime/langgraph/inbox_loop.py)
...
| P8（验收阶段已完成） | 灰度能力验证（不含产品 R5 删除） | 双 Runtime 完整矩阵、持久 LangGraph checkpoint、灰度路由 | 38/38 可执行、56/56 验收全绿；`runtime_engine` 固定归属与可回退默认路由；ThinkLife compatibility host 仍保留 |
...
- Scenario CLI、Web 分层工作区、结构化报告和
  [ThinkLife P1 分层差距表](think-life-p1-gap-matrix.zh-CN.md)均已完成；
...
- [contracts.py](../../../src/m_agent/runtime/domain/contracts.py)
- [transaction/domain.py](../../../src/m_agent/runtime/domain/contracts.py)
- [transaction/store.py](../../../src/m_agent/runtime/transaction/store.py)
- [transaction/uow.py](../../../src/m_agent/runtime/transaction/uow.py)
- [transaction/schedule.py](../../../src/m_agent/runtime/transaction/schedule.py)
- [transaction/flush.py](../../../src/m_agent/runtime/transaction/flush.py)
- [transaction_registry.py](../../../src/m_agent/runtime/transaction/registry.py)
- [test_think_life_tx_foundation.py](../../../tests/runtime/test_transaction_foundation.py)
...
### P8：验收层灰度能力（产品 R5 收缩不包含在内）

#### 当前状态

P8 的共享验收矩阵与灰度路由能力已完成。这里的“完成”不表示产品侧 ThinkLife
已经删除；产品稳定观察与旧 host 退出分别由 R4、R5 管理。实现摘要：
...
P8 后生产接线进一步提供 `CompositeRuntimeHost`：两个 engine 同时打开，按照
transaction 在持久 Store 中的 `runtime_engine` 分发。产品 YAML 默认
`langgraph_v1`；部署 env 可覆盖该默认值，但不能改变已有 transaction 的归属。
...
- 双 Runtime 完整矩阵可以比较；
- 共享 Gate 中没有状态串扰、重复效果或 Scene/flush 漂移；
...
- R4 接线已完成：新 transaction 产品默认 LangGraph、checkpoint fail-fast/health、
  原 transaction Schedule、统一原子 Flush 与双 engine 归属路由均已落地。
- R4 观察期仍需持续执行：
  `python scripts/run_runtime_migration_gate.py --rounds 3`，并进行真实流量观察和 env
  回退演练。
- 每日/发布前应执行只读
  `python scripts/audit_runtime_cutover.py --root data/memory/chat-api`；报告中的 blockers
  包括旧 Store 非终态 transaction、Stimulus、Schedule、effect、未完成 outbox，以及
  `composite-flush.sqlite3` 中未完成的 engine commit/materialization。
- R5 尚未开始。只有观察期完成并且 audit 返回
  `legacy_retirement_ready=true`（可用 `--require-legacy-retired` 强制退出门）后，才允许
  删除 ThinkLife transaction 内循环。
...
P8 与 R4 生产机制已完成，R4 观察和 R5 退出门尚未完成。下一步按以下顺序推进：

1. 在 CI、每日和发布前运行 `scripts/run_runtime_migration_gate.py`，归档其 JSON 结果。
2. 对实际持久化根目录运行 `scripts/audit_runtime_cutover.py`，持续消化 legacy blockers。
3. 演练把 `M_AGENT_DEFAULT_RUNTIME_ENGINE` 切回 `think_life_v1`，验证仅新 transaction
   改路由，两个 engine 的存量 transaction 都能继续完成。
4. 达到约定观察期且 `legacy_retirement_ready=true` 后，再制定并执行 R5 删除清单；
   在此之前保留 Composite host 与 ThinkLife compatibility runtime。
5. 视需要并行接线前端 transaction 控件，但这不是 Runtime Gate 的前置条件。
...
| [langgraph-runtime-migration-plan.zh-CN.md](langgraph-runtime-migration-plan.zh-CN.md) | 记录 LangGraph 迁移边界、风险和 P0～P8 实施路径 | P8 完整矩阵已完成；R4 机制已接线、观察中；R5 等待 audit 退出门 |
| [semantic-acceptance-platform.zh-CN.md](semantic-acceptance-platform.zh-CN.md) | 说明当前验收平台的使用方法、P1 合同状态和旧 Supporting 目录 | 已同步完整 P1 Harness、35 个 variant、CLI/Web/report 和 UI 分层工作区 |
| [think-life-p1-gap-matrix.zh-CN.md](think-life-p1-gap-matrix.zh-CN.md) | 记录 ThinkLife 对 P1 全矩阵的分层执行结果和已登记差距 | P1 基线：35 可执行、35 Known Gap、0 未登记失败 |
