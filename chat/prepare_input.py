...
def emerge_chat_prompt_wo_memory(history,observation = None): 
    if observation:
        return f"{ROLE}{SELFWOM.format(LANGUAGE)}{OBSERVATION.format(observation)}{HISTORY.format(history)}"
    else:
        return f"{ROLE}{SELFWOM_WO_OB.format(LANGUAGE)}{HISTORY.format(history)}"
    
# 0922：产生正常对话场景的记忆性对话
def emerge_chat_prompt_w_memory(history,memory,observation = None): 
    if observation: 
        return f"{ROLE}{SELFWOM.format(LANGUAGE)}{MEMORY.format(memory)}{OBSERVATION.format(observation)}{HISTORY.format(history)}"
    else: 
        return f"{ROLE}{SELFWOM_WO_OB.format(LANGUAGE)}{MEMORY.format(memory)}{HISTORY.format(history)}"
