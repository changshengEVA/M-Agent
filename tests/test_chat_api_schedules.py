...
def test_schedule_crud_endpoints(tmp_path: Path) -> None:
    ...
    with TestClient(app) as client:
        ...
        assert response.json()["heartbeat"]["scheduler"]["beat_interval_seconds"] == 10

        heartbeat = client.get("/v1/chat/threads/demo-thread/schedules/heartbeat")
        assert heartbeat.status_code == 200
        assert heartbeat.json()["heartbeat"]["scheduler"]["beat_interval_seconds"] == 10
        assert heartbeat.json()["heartbeat"]["worker"]["alive"] is True
