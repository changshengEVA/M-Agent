    ...
    <section class="panel toolbar">
      <div class="toolbar-grid">
        <div>
          ...
          <input id="configInput" type="text" value="config\agents\memory\locomo_eval_memory_agent.yaml" />
        </div>
        <div>
...
class TraceRunState:
    ...
    def _reset_locked(self) -> None:
        self.running = False
        self.status = "Ready"
        self.config = r"config\agents\memory\locomo_eval_memory_agent.yaml"
...
def _resolve_config_path(config_text: str) -> Path:
    config_path = Path(config_text or r"config\agents\memory\locomo_eval_memory_agent.yaml")
    if not config_path.is_absolute():
        config_path = PROJECT_ROOT / config_path
    return config_path
    ...
    class RequestHandler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:
            ...
            if path == "/api/run":
                ...
                config_text = str(payload.get("config", r"config\agents\memory\locomo_eval_memory_agent.yaml") or "").strip()
                question = str(payload.get("question", "") or "").strip()
